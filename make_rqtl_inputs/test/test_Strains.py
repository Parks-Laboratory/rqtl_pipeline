...
male_sex_index = Global.SEX_LABEL_AS_NUMERIC[Global.MALE]
...
class test_averaged(unittest.TestCase):
	def test_append_simple(self):
		...
		strain = 'Husky'
		sex = 'male'
		male_sex_index = Global.SEX_LABEL_AS_NUMERIC[sex]
	...
	def test_append(self):
		...
		strain = 'Husky'
		self.line = ['Jake', strain, Global.MALE, '-', '0', '1', '2', '4', '8']
		strains.append(self.line)
		...
		for index, phenotype in enumerate(self.line[3:]):
			if phenotype == Global.RQTL_MISSING_VALUE_LABEL:
				self.assertEqual(male_husky.rows[index], self.line[3+index])
			else:
				self.assertEqual(male_husky.rows[index], [self.line[3+index]])

		# add 2 more male huskies, make sure data stored correctly
		strains.append(['Franco', strain, Global.MALE, '0', '-', '1', '2', '6', '3'])
		strains.append(['Sam', strain, Global.MALE, '2', '-', '1', '-', '7', '5'])
		...
		strains.append(['Ellie', strain, Global.FEMALE, '-', '-', '-', '2.2', '.6', '-'])
		strains.append(['Nancy', strain, Global.FEMALE, '3.0', '-', '1.1', '-', '7', '2'])
		strains.append(['Irene', strain, Global.FEMALE, '3.2E-1', '-', '-', '7', '.7', '1'])
		...
		strain = 'Husky'
		strains.append(['Ellie', strain, Global.FEMALE, '-', '-', '-', '-', '-', '-'])
		strains.append(['Sam', strain, Global.MALE, '-', '-', '-', '-', '-', '-'])
		self.assertEqual(female_husky.rows, [['3.0','3.2E-1'], '-', ['1.1'], ['2.2','7'],
			['.6','7','.7'], ['2','1'], female_sex_index, 'Husky.f'])
		self.assertEqual(female_husky.rows, [['3.0','3.2E-1'], '-', ['1.1'], ['2.2','7'],
			['.6','7','.7'], ['2','1'], female_sex_index, 'Husky.f'])
...
class test_not_averaged(unittest.TestCase):
	...
	def test_append(self):
		...
		# add 2 more male huskies, make sure data stored correctly
		franco_line = ['Franco', strain, Global.MALE, '0', '-', '1', '2', '6', '3']
		sam_line = ['Sam', strain, Global.MALE, '2', '-', '1', '-', '7', '5']
		...
		# add female huskies, make sure data stored correctly
		ellie_line = ['Ellie', strain, Global.FEMALE, '-', '-', '-', '2.2', '.6', '-']
		nancy_line = ['Nancy', strain, Global.FEMALE, '3.0', '-', '1.1', '-', '7', '2']
		irene_line = ['Irene', strain, Global.FEMALE, '3.2E-1', '-', '-', '7', '.7', '1']
		...
		# add another strain, make sure data stored correctly for that strain
		strain = 'Main_Coon'
		jacen_line = ['Jacen', strain, Global.MALE, '0', '-', '1', '2', '6', '3']
		jaina_line = ['Jaina', strain, Global.FEMALE, '3.0', '-', '1.1', '-', '7', '2']
		...
		strain = 'Husky'
		ellie_2_line = ['Ellie', strain, Global.FEMALE, '-', '-', '-', '-', '-', '-']
		sam_2_line = ['Sam', strain, Global.MALE, '-', '-', '-', '-', '-', '-']
