...
* Finally, there are 4 types of files generated by each mapping: 
	...
	* **.pdf** file has the LOD plots with all markers and all chromosomes (if pdf='TRUE' in map.sh)
			(.pdf used instead of .png b/c chtc nodes can't make .pngs out of the box)
	* **.rdata** file has the R/QTL scanone object, which can be loaded back (if rdata='TRUE' in map.sh)
